...
class BoxKernel(Kernel):
    ...
    def banner(self):
        if self._banner is None:
            self._banner = check_output(['box', '--version']).decode('utf-8')
        return self._banner

    language_info = {'name': 'box',
    ...
    def _start_box(self):
        ...
        # so that box and its children are interruptible.
        sig = signal.signal(signal.SIGINT, signal.SIG_DFL)
        try:
            # Note: the next few lines mirror functionality in the
            # box() function of pexpect/replwrap.py.  Look at the
            # source code there for comments and context for
            # understanding the code here.
            child = pexpect.spawn("../../bin/box.js", echo=False, encoding='utf-8', codec_errors='replace')
            ...
            self.boxwrapper = IREPLWrapper(child, u'\$', prompt_change,
        ...
        self.boxwrapper.run_command(image_setup_cmd)
    ...
    def do_execute(self, code, silent, store_history=True,
        ...
        try:
            ...
            self.boxwrapper.run_command(code.rstrip(), timeout=None)
        except KeyboardInterrupt:
            self.boxwrapper.child.sendintr()
            interrupted = True
            self.boxwrapper._expect_prompt()
            output = self.boxwrapper.child.before
            self.process_output(output)
        except EOF:
            output = self.boxwrapper.child.before + 'Restarting Box'
            self._start_box()
            self.process_output(output)
        ...
        try:
            exitcode = int(self.boxwrapper.run_command('echo $?').rstrip())
        except Exception:
            exitcode = 1
    ...
    def do_complete(self, code, cursor_pos):
        ...
        if token[0] == '$':
            ...
            output = self.boxwrapper.run_command(cmd).rstrip()
